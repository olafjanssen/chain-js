...
Chain.prototype.channel = new HydnaChannel('http://###yourchannel###.hydna.net', 'rw');
Chain.prototype.position = 0;
 ...
 */
Chain.prototype.start = function (pos, callback) {
    'use strict';
    ...
    this.channel.onopen = function () {
        Chain.prototype.channel.send('p' + Chain.prototype.position);
    };

    // listen to messages and respond if the current link in the chain is that of the user
    this.channel.onmessage = function (event) {
        if (event.data === 'p' + Chain.prototype.position) {
            callback();
        }
    };
};
